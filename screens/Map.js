...
export default function ParkingMap() {
  ...
  useEffect(() => {
    ...
    parkingsSpots.forEach(parking => {
      initialHours[parking.id] = 1;
    });
  ...
  const RenderHeader = () => {
    return (
      <View style={styles.header}>
        ...
        >
          <TouchableWithoutFeedback>
            <Ionicons name="menu" size={theme.SIZES.icon * 1.5} />
          </TouchableWithoutFeedback>
        </View>
      </View>
    );
  ...
  const renderHours = (id) => {
    ...
    return (
      <Dropdown
        ...
        onSelect={(index, value) => handleHours(id, value)}
        renderRow={option => (
    ...
    <View style={styles.container}>
      ...
      >
        {parkingsSpots.map(parking => (
          ...
          >
            <TouchableWithoutFeedback
              onPress={() => setActive({ active: parking.id })}
            >
              <View
                style={[
                  styles.marker,
